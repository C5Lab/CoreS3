...
VERSION = "v01"
DEFAULT_BAUD = 460800
REQUIRED_FILES = ["bootloader.bin", "partition-table.bin", "coreS3.bin"]
...
def do_flash(port, baud=DEFAULT_BAUD, flash_mode="dio", flash_freq="80m"):
    cmd = [
        ...
        "--flash-freq",
        flash_freq,
        OFFSETS["bootloader.bin"],
        "bootloader.bin",
        OFFSETS["partition-table.bin"],
        "partition-table.bin",
        OFFSETS["coreS3.bin"],
        "coreS3.bin",
    ]
...
def main():
    parser = argparse.ArgumentParser(description="ESP32-S3 flasher with robust reboot handling")
    parser.add_argument("--version", action="version", version=f"%(prog)s {VERSION}")
    parser.add_argument("--port", help="Known serial port (e.g., COM13 or /dev/ttyACM0)")
    parser.add_argument(
        "baud",
        nargs="?",
        type=int,
