...
static const char *nfc_bus_cmd(nfc_bus_mode_t mode);
...
static void on_nfc_mode_changed(lv_event_t *e);
static void on_settings_screen_deleted(lv_event_t *e);

static const char *nfc_bus_cmd(nfc_bus_mode_t mode)
{
    switch (mode) {
    case NFC_BUS_MODE_I2C:   return "i2c";
    case NFC_BUS_MODE_PN532: return "pn532";
    default:                 return "spi";
    }
}
...
static void settings_nfc_mode_sync_ui(void)
{
    if (s_nfc_mode_dd && lv_obj_is_valid(s_nfc_mode_dd)) {
        s_nfc_mode_ignore_event = true;
        lv_dropdown_set_selected(s_nfc_mode_dd, (uint32_t)nfc_bus_mode);
...
static void settings_nfc_mode_set_busy(bool busy)
{
    ...
    if (busy) {
        lv_obj_add_state(s_nfc_mode_dd, LV_STATE_DISABLED);
    } else {
        lv_obj_clear_state(s_nfc_mode_dd, LV_STATE_DISABLED);
    }
}
...
static void settings_nfc_mode_request_set(nfc_bus_mode_t mode)
{
    ...
    snprintf(cmd, sizeof(cmd), "set_nfc_bus %s", nfc_bus_cmd(mode));
...
static bool parse_nfc_bus_line(const char *line, nfc_bus_mode_t *mode_out)
{
    ...
    if (tolower((unsigned char)value[0]) == 'p' &&
        tolower((unsigned char)value[1]) == 'n' &&
        value[2] == '5' && value[3] == '3' && value[4] == '2') {
        *mode_out = NFC_BUS_MODE_PN532;
        return true;
    }
...
static void on_nfc_mode_collect_complete(const char **lines, int count)
{
    nfc_uart_action_t action = s_nfc_uart_action;
    nfc_bus_mode_t parsed_mode = nfc_bus_mode;
    bool parsed = false;
    bool nfc_detected = false;
    bool nfc_probed = false;

    if (action == NFC_UART_ACTION_INIT) {
        for (int i = 0; i < count; i++) {
            if (!lines[i]) continue;
            if (strstr(lines[i], "[NFC] not detected")) {
                nfc_probed = true;
                nfc_detected = false;
                break;
            }
            if (strstr(lines[i], "[NFC] detected")) {
                nfc_probed = true;
                nfc_detected = true;
                break;
            }
        ...
        if (!parsed && action == NFC_UART_ACTION_LOAD) {
            ESP_LOGW(TAG, "NFC bus response missing bus line");
        }
    }

    s_nfc_uart_result.action = action;
    s_nfc_uart_result.parsed = parsed;
    s_nfc_uart_result.mode = parsed_mode;
    s_nfc_uart_result.nfc_detected = nfc_detected;
    s_nfc_uart_result.nfc_probed = nfc_probed;
    s_nfc_uart_action = NFC_UART_ACTION_NONE;
...
{
    (void)user_data;
    nfc_uart_action_t action = s_nfc_uart_result.action;

    if (action == NFC_UART_ACTION_INIT) {
        if (s_nfc_uart_result.nfc_probed) {
            device_info_set_has_nfc(s_nfc_uart_result.nfc_detected);
        }
        if (s_settings_screen_active) {
    ...
    if (action == NFC_UART_ACTION_SET) {
        if (s_settings_screen_active) {
            settings_nfc_mode_sync_ui();
        }
        settings_nfc_mode_begin_collect(NFC_UART_ACTION_INIT, "init_nfc");
        return;
    }
...
static void on_nfc_mode_changed(lv_event_t *e)
{
    ...
    nfc_bus_mode_t selected = (nfc_bus_mode_t)lv_dropdown_get_selected(dd);
    ...
    if (selected > NFC_BUS_MODE_PN532) {
        settings_nfc_mode_sync_ui();
        return;
    }
...
void load_settings_from_nvs(void)
{
    ...
    if (err == ESP_OK && nbus <= NFC_BUS_MODE_PN532) {
...
void show_settings_screen(void)
{
    ...
    s_nfc_mode_dd = lv_dropdown_create(nfc_row);
    lv_dropdown_set_options(s_nfc_mode_dd, "SPI\nI2C\nPN532");
    lv_dropdown_set_selected(s_nfc_mode_dd, (uint32_t)nfc_bus_mode);
    lv_obj_set_width(s_nfc_mode_dd, 110);
    lv_obj_set_style_text_font(s_nfc_mode_dd, &lv_font_montserrat_12, 0);
    lv_obj_set_style_bg_color(s_nfc_mode_dd, ui_card_color(), 0);
    lv_obj_set_style_text_color(s_nfc_mode_dd, ui_text_color(), 0);
    lv_obj_set_style_border_color(s_nfc_mode_dd, ui_border_color(), 0);

    lv_obj_t *nfc_list = lv_dropdown_get_list(s_nfc_mode_dd);
    if (nfc_list) {
        lv_obj_set_style_bg_color(nfc_list, ui_card_color(), 0);
        lv_obj_set_style_text_color(nfc_list, ui_text_color(), 0);
        lv_obj_set_style_border_color(nfc_list, ui_border_color(), 0);
        lv_obj_set_style_text_font(nfc_list, &lv_font_montserrat_12, 0);
    }

    lv_obj_add_event_cb(s_nfc_mode_dd, on_nfc_mode_changed,
